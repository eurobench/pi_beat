...
file_id=fopen(strcat(pwd,"/", name2, "_PL_p", ".yaml"),'w'); %%open file to write into
fprintf(file_id, "type: 'vector' 'measure unit: m' \n");
label_str="direction: [";
...
file_id=fopen(strcat(pwd,"/", name2, "_EA_p", ".yaml"),'w'); %%open file to write into
fprintf(file_id, "type: 'vector' 'measure unit: m^2' \n");
label_str="direction: [";
for i=1:length(direction)
  label_str=sprintf("%s",label_str,char(direction(i)));
  if i!=length(direction)
